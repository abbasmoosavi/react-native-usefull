        ...
        return (
            <View
                ...
                <View style={{ width: '100%', flexDirection: 'column', height: '35%', paddingVertical: wp('2%'), paddingHorizontal: wp('4%') }}>
                    ...
                    {/* عنوان */}
                    <Text style={[{ color: textColor, textAlign: I18nManager.isRTL ? 'left' : 'right', width: '100%', fontSize, lineHeight: UFRN.FontSize.SUBTITLE_HEIGHT, alignSelf: 'flex-start' }, titleStyle]}>{title}</Text>

                    {/* توضیح کوتاه سه خطی */}
                    <Text numberOfLines={3} ellipsizeMode={"tail"} style={[{ marginTop: hp('0.5%'), textAlign: I18nManager.isRTL ? 'left' : 'right', width: '100%', lineHeight: hp('3%'), color: textColor, fontSize, alignSelf: 'center' }, textStyle]}>{text}</Text>
